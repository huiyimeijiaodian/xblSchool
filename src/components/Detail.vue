<template>
  <div class="content">
    ...
    <div class="s_content">
      ...
      <pay-wrap ref="pay" :payObj="this.payObj" :havewx="this.selSeries.havewx" :coins="this.coins" :huaweipay="this.selSeries.huaweipay"></pay-wrap>
    </div>
  </div>
    ...
    created() {
      ...
      // axios ajax请求
        this.$http.get('../../series.json').then((m) => {
          ...
          this.selSeries = Object.assign({}, this.selSeries, m.data);
          this.getData();
        });
      // vue-resource jsonp请求
//      this.$http.jsonp('http://wxxbl.youban.com/app/school/detail.jsonp', { // 请求参数
//          params: {'as': as, 'debug': '9579141'}, // 3014982 9579141
//          jsonp: 'jsoncallback'
//        }).then(function(res) {
//        this.$refs.loadmask.colseLoadmask();
//        console.log('detail详情页', JSON.parse(res.bodyText));
//        this.selSeries = Object.assign({}, this.selSeries, JSON.parse(res.bodyText));
//        this.getData();
//      }, function() {
//        console.log(1);
//      });
...
<style rel="stylesheet/css">
  ...
  /*.series_scroll{height:100%;overflow: hidden;}*/
  .headimg_box,.Hmainbox{ clear: both; height: auto; overflow: hidden; }
  .Hmainbox{ padding-bottom: 50px;margin-bottom: 30px;background: white; }
  ...
  .IconRedcom:before{ content: "";width:0; position: absolute;left: 0; top: 25px;   height:0;    border-right:16px solid transparent;   border-top:10px solid #00a0e9; }
  .IconRedcom:after{ content: "";width:0; position: absolute; top: 25px; right: 0;  height:0;    border-bottom:10px solid transparent;   border-right:16px solid #00a0e9; }
